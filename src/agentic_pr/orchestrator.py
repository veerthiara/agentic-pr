    ...
    try:
        ...
        if not preflight.ok:
            return _blocked(
                ...
                comment=preflight_blocked_comment(run_id, preflight.reason or "preflight_failed", preflight.details),
            )
        ...
        if aider_result.timed_out:
            ...
            _write_record(config, issue=issue, run_id=run_id, branch=branch, status="failed", pr_url=None, started_at=started_at, finished_at=_now_iso(), log_file=log_file, error_summary=f"Aider timed out after {config.aider_timeout_seconds} seconds")
        ...
        if not has_changes(config.repo_path):
            ...
            _write_record(config, issue=issue, run_id=run_id, branch=branch, status="no_changes", pr_url=None, started_at=started_at, finished_at=_now_iso(), log_file=log_file, error_summary=None)
        ...
        if not safety.ok:
            return _blocked(
                ...
                started_at=started_at,
                log_file=log_file,
                reason=safety.reason or "safety_failed",
                details=safety.details,
                comment=blocked_comment(run_id, safety.reason or "safety_failed", safety.details),
            )

        stage = "lint"
        validation = _run_validation(config, config.lint_cmd, "lint")
        if validation is not None:
            return _failed_validation(config, issue, run_id, branch, started_at, log_file, "lint", validation)

        stage = "test"
        validation = _run_validation(config, config.test_cmd, "test")
        if validation is not None:
            return _failed_validation(config, issue, run_id, branch, started_at, log_file, "test", validation)

        stage = "commit"
        commit_all(config.repo_path, f"REV06: work on issue #{issue.number}")

        stage = "push"
        push_branch(config.repo_path, branch)

        stage = "create_pr"
        body = pr_body(config=config, issue=issue, run_id=run_id, branch=branch, log_file=log_file or "", aider_exit_code=aider_result.exit_code)
        ...
        _write_record(config, issue=issue, run_id=run_id, branch=branch, status="pr_created", pr_url=pr_url, started_at=started_at, finished_at=_now_iso(), log_file=log_file, error_summary=None)
        ...
        if issue is not None and run_id is not None:
            ...
            _write_record(config, issue=issue, run_id=run_id, branch=branch, status="failed", pr_url=pr_url, started_at=started_at, finished_at=_now_iso(), log_file=log_file, error_summary=error_summary)
...
def _blocked(config: AgentConfig, *, issue, run_id: str, branch: str, started_at: str, log_file: str | None, reason: str, details: list[str], comment: str) -> RunResult:
    mark_blocked(config, issue.number)
    comment_issue(config, issue.number, comment)
    error_summary = f"{reason}: {'; '.join(details[:3])}"
    _write_record(config, issue=issue, run_id=run_id, branch=branch, status="blocked", pr_url=None, started_at=started_at, finished_at=_now_iso(), log_file=log_file, error_summary=error_summary)
    return RunResult("blocked", error_summary, issue_number=issue.number, run_id=run_id)


def _failed_validation(config: AgentConfig, issue, run_id: str, branch: str, started_at: str, log_file: str | None, command_name: str, error_summary: str) -> RunResult:
    mark_failed(config, issue.number)
    if config.comment_on_failure:
        comment_issue(config, issue.number, validation_failed_comment(run_id, command_name, error_summary))
    _write_record(config, issue=issue, run_id=run_id, branch=branch, status="failed", pr_url=None, started_at=started_at, finished_at=_now_iso(), log_file=log_file, error_summary=error_summary)
    return RunResult("failed", error_summary, issue_number=issue.number, run_id=run_id)


def _write_record(config: AgentConfig, *, issue, run_id: str, branch: str, status: str, pr_url: str | None, started_at: str, finished_at: str | None, log_file: str | None, error_summary: str | None) -> None:
    write_run_record(config.run_record_dir, RunRecord(run_id=run_id, issue_number=issue.number, issue_title=issue.title, owner_repo=config.owner_repo, model=config.model, base_branch=config.base_branch, agent_branch=branch, status=status, pr_url=pr_url, started_at=started_at, finished_at=finished_at, log_file=log_file, error_summary=error_summary))
