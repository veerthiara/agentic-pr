...
def run_once(config: AgentConfig) -> str:
    ...
    if issue is None:
        return f"No open issues found with label: {config.label_todo}"
    ...
    try:
        ...
        if not has_changes(config.repo_path):
            ...
            return "Aider produced no changes."

        commit_all(config.repo_path, f"REV02: work on issue #{issue.number}")
