...
from agentic_pr.engines.base import EngineRequest, EngineResult
    ...
    def test_get_engine_rejects_unknown_engine(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(Exception) as cm:
                get_engine(config)
            self.assertIn("Unsupported ENGINE=unknown", str(cm.exception))
            self.assertIn("Supported engines: aider", str(cm.exception))

    def test_aider_engine_builds_expected_command(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            engine = AiderEngine()
            ...
            # Check that the engine builds the correct command
            # We can't easily test the full run without mocking, but we can verify
            # the engine is properly configured
            self.assertEqual(engine.name, "aider")
